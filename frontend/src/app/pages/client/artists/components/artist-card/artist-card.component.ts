import { Component, Input, inject } from '@angular/core';
import { CommonModule } from '@angular/common';
import { Router } from '@angular/router';
import { BookingStateService } from '../../../booking/services/booking-state.service';

@Component({
    selector: 'app-artist-card',
    standalone: true,
    imports: [CommonModule],
    template: `
    <div class="flex flex-col bg-white dark:bg-gray-800 rounded-[12px] p-6 shadow-sm hover:shadow-xl hover:-translate-y-2 transition-all duration-300 group h-full">
        <div class="relative w-32 h-32 mx-auto mb-4">
            <div class="w-full h-full rounded-full overflow-hidden border-4 border-[#fcf8f9] shadow-inner flex items-center justify-center bg-[#ffc1d0]">
                <span class="text-4xl font-bold text-[#1c0d10] font-serif">{{ (name || '').charAt(0).toUpperCase() }}</span>
            </div>
        </div>
        <div class="flex flex-col items-center gap-1 flex-grow">
            <h3 class="text-text-main dark:text-white text-2xl font-bold font-serif">{{ name }}</h3>
            <div class="flex flex-wrap items-center justify-center gap-1.5 mt-1">
                <div *ngFor="let tag of specialtyTags" class="bg-[#fae8eb] dark:bg-primary/20 px-3 py-1 rounded-full">
                    <span class="text-primary dark:text-primary-light text-xs font-bold uppercase tracking-wide font-display-2">{{ tag }}</span>
                </div>
            </div>
            <div class="flex items-center gap-1 mt-2 text-yellow-500">
                <span class="material-symbols-outlined text-xl fill-current">star</span>
                <span class="text-text-main dark:text-white font-bold text-sm font-display-2">{{ rating }}</span>
                <span class="text-text-muted dark:text-gray-400 text-xs font-display-2">({{ reviews }} reviews)</span>
            </div>
        </div>
        <button (click)="bookArtist()" class="w-full mt-6 bg-primary hover:bg-primary/90 text-white font-bold py-3 px-4 rounded-xl transition-colors shadow-lg shadow-primary/20 flex items-center justify-center gap-2 font-display-2">
            <span>Book with {{ name.trim().split(' ')[0] }}</span>
        </button>
    </div>
  `
})
export class ArtistCardComponent {
    private router = inject(Router);
    private bookingStateService = inject(BookingStateService);

    @Input() id: string = '';
    @Input() name: string = '';
    @Input() specialty: string = '';
    @Input() rating: number = 5.0;
    @Input() reviews: number = 0;
    @Input() image: string = '';
    @Input() badgeIcon?: string;
    
    get specialtyTags(): string[] {
        if (!this.specialty) return ['STYLIST'];
        return this.specialty.split(',').map(s => s.trim()).filter(Boolean);
    }

    bookArtist() {
        if (this.id) {
            // Reconstruct artist object for the state service
            const artistObj = {
                _id: this.id,
                name: this.name,
                specialty: this.specialty,
                rating: this.rating,
                reviews: this.reviews,
                imageUrl: this.image
            };
            this.bookingStateService.updateState({ artist: artistObj as any });
        }
        this.router.navigate(['/client/booking/services']);
    }
}
